...
public class GuiHelper implements IGuiHelper
{
                              ...
                              String displayString, Color color)
    {
        ...
        if (!stack.isEmpty())
        {
            ...
            GlStateManager.multiTexCoord2f(GL13.GL_TEXTURE1, short1 / 1.0F, short2 / 1.0F);
                                     ...
                                     Consumer<List<ITextComponent>> tooltipModifier)
    {
        ...
        GuiUtils.drawHoveringText(stack, list.stream().map(ITextComponent::getFormattedText).collect(toList()),
                mouseX, mouseY, this.mc.getMainWindow().getWidth(), this.mc.getMainWindow().getHeight(),
                this.mc.getMainWindow().getWidth(), this.mc.fontRenderer);
    }
    ...
    public void translateVecToScreenSpace(Vector2i vec)
    {
        ...
        if (currentScreen != null)
        {
            vec.setX((int) (vec.getX() / ((float) currentScreen.width / this.mc.getMainWindow().getWidth())));
            vec.setY((int) ((currentScreen.height - vec.getY()) / ((float) currentScreen.height / this.mc.getMainWindow().getHeight())
                    - 1));
        }
    }
    ...
    private void enableAlpha()
    {
        ...
        GlStateManager.blendFuncSeparate(
                SourceFactor.SRC_ALPHA.param,
                DestFactor.ONE_MINUS_SRC_ALPHA.param,
                SourceFactor.ONE.param,
